...
if var in genotipos_unicos:
    ...
    try:
        ...
        def pubmed(geno_pub):
            ...
            record6 = Entrez.read(handle2)
            print("IDs de la variante en pubmed: ", record6['IdList'])
            handle2 = Entrez.esummary(db="pubmed", id=record6['IdList'][0])
            print("Descripción del primer id (", record6['IdList'][0], ") de la variante en pubmed: ",
                  Entrez.read(handle2))
            handle2.close()

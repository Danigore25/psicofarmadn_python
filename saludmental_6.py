# PARTE FINAL DEL PROGRAMA DE PYTHON APP: LECTURA DE UN ARCHIVO PDF
...
# 3. Lista de genes de importancia clínica en salud mental
...
# 6. Buscar genes y genotipos * en archivos
...
if var in genotipos_unicos:
    for g in genes:
        if g in var:
            handle = Entrez.esearch(db="gene", term=g, sort="relevance")
            record5 = Entrez.read(handle)
            handle = Entrez.esummary(db="gene", id=record5['IdList'][0])
            record6 = Entrez.read(handle)
            print("Gene data: ", record6['DocumentSummarySet']['DocumentSummary'])
            handle.close()
    try:
